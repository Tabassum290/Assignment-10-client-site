        ...
        .then((data) => {
            if (data.insertedId) {
                ...
                .catch((error) => {
                    ...
                    Swal.fire("Error", "Failed to update recommendation count.", "error");
                });
            }
        })
        .catch((error) => {
            console.error("Error adding recommendation:", error);
            Swal.fire("Error", "Failed to add recommendation. Try again later.", "error");
        });
    };
...
<div className='my-3 p-4 '>
    <img className='w-3/4 h-[360px] mx-auto' src={selectedQuery.productImage} alt="" />
</div>
<hr></hr>

<div className='p-4 '>
    <h1 className='text-2xl'>{selectedQuery.queryTitle}</h1>
     <p className='text-black text-lg my-2'>Reason For Boycott: {selectedQuery.boycottingReason}</p>
...
<section className="p-4 max-w-7xl mx-auto">
                <h2 className="text-3xl font-bold text-center mb-6">Add Recommendation For :{selectedQuery.productName}</h2>
                <form onSubmit={handleAddRecommendation} className="bg-gray-100 p-6 rounded-lg shadow-lg max-w-xl mx-auto">
                    <div className="mb-4">
                        <label className="block text-sm font-bold mb-2">Recommendation Title</label>
                        <input
                            type="text"
                    ...
                    <div className="mb-4">
                        <label className="block text-sm font-bold mb-2">Recommended Product Name</label>
                        <input
                            type="text"
                    ...
                    <div className="mb-4">
                        <label className="block text-sm font-bold mb-2">Recommended Product Image</label>
                        <input
                            type="url"
                    ...
                    <div className="mb-4">
                        <label className="block text-sm font-bold mb-2">Recommendation Reason</label>
                        <textarea
            ...
            <section className="p-4 max-w-7xl mx-auto">
                <h3 className="text-2xl font-semibold text-center mb-4">All Recommendations</h3>
                {recommendations.length === 0 ? (<p>No recommendations yet.</p>) : 
                (
                    recommendations.map((rec) => <AllReccomendations key={rec._id} rec={rec}></AllReccomendations>
                ))}
            </section>
            <Footer />
        </div>
